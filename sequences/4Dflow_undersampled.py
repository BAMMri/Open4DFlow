...
def setup_sampling(Ny, Nz, fov, undersampling_factor=8, center_percent_y=24, center_percent_z=23, seed=32):
    np.random.seed(seed)
...
def generate_phase_samples(Ny, Nz, fov, num_phases, undersampling_factor=8, center_percent_y=24, center_percent_z=23, base_seed=32):
    phase_samples = []
...
if __name__ == "__main__":
    ...
    if DO_TRIGGERING:
        # max_phases = int(math.floor(TRIG_TIME / (4 * seq.TR)))
