...
def setup_sampling(Ny, Nz, fov, undersampling_factor=9, center_percent_y=25, center_percent_z=20, seed=32):
    np.random.seed(seed)
...
def generate_phase_samples(Ny, Nz, fov, num_phases, undersampling_factor=9, center_percent_y=25, center_percent_z=20, base_seed=32):
    phase_samples = []
...
class MRISequence:
    def __init__(self, TE, TR, fov, Nx, Ny, Nz, Nslices, venc, slice_thickness, alpha, bandwidth, tbw, heart_rate):
        ...
        self.sys = pp.Opts(max_grad=41, grad_unit='mT/m', max_slew=110, slew_unit='T/m/s',
    ...
    def make_tr(self, areay, areaz, m1x, m1y, m1z, labels=None):
        ...
        gy_vel = self.make_gradient('y', y_moment_params, flow_g_start=flow_g_start,
                                    additional_params={'gmax': 41, 'smax': 110.0})
...
if __name__ == "__main__":
    ...
    FOV = [300e-3, 220e-3, 200e-3]
    VENC = 0.25



    seq = MRISequence(TE=4.7e-3, TR=7.2e-3, fov=FOV, Nx=int(np.ceil(FOV[0] / RESOLUTION[0])),Ny=int(np.ceil(FOV[1] / RESOLUTION[1])),Nz=int(np.ceil(FOV[2] / RESOLUTION[2])),
                      Nslices=6, venc=VENC, slice_thickness=200e-3, alpha=10, bandwidth=1e3, tbw=2, heart_rate=TRIG_TIME)
    ...
    trig = pp.make_trigger(channel="physio1",  duration=1000e-6)
